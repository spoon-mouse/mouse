        ...
        switch (option){
            ...
            case HAND:
                throw new IllegalArgumentException(
                        "HAND mode needs a resolved UTXO list — use setCoinSelector(CoinSelector) instead, "
                        + "constructed by the UI after prompting for which UTXOs to use.");
        }
    }

    /**
     * Accepts any caller-supplied selection strategy directly — e.g. a UI-built
     * ManualCoinSelector already populated with resolved UTXO choices. TxnUtil
     * never needs to know that strategy exists; it just uses whatever CoinSelector
     * it's given.
     */
    public void setCoinSelector(CoinSelector customSelector) {
        coinSelector = customSelector;
    }
